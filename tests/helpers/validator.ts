/**
 * Test validator lifecycle management
 *
 * Based on pattern from onchain-verifier/tests/test-validator.test.ts
 */
...
export function getProgramPath(): string {
  ...
  if (!fs.existsSync(programPath)) {
    throw new Error(
      `Program not found at ${programPath}. Run 'cargo build-sbf' first.`
    );
  }
  return programPath;
}
...
export function getProgramId(): PublicKey {
  const keypairPath = path.join(__dirname, '../../target/deploy/solana_poker-keypair.json');
  if (fs.existsSync(keypairPath)) {
    const keypairData = JSON.parse(fs.readFileSync(keypairPath, 'utf-8'));
    const keypair = Keypair.fromSecretKey(new Uint8Array(keypairData));
    return keypair.publicKey;
  }
  // Fallback: generate a deterministic program ID
  // In practice, the keypair file should exist after cargo build-sbf
...
export async function startValidator(): Promise<void> {
  ...
  // Spawn validator with program pre-loaded
  ...
  // Don't let the validator block Node exit
  ...
  if (!programAccount) {
    throw new Error(`Program account not found after validator startup. ID: ${programId.toBase58()}`);
  }
 ...
 */
export async function createFundedPayer(lamports: number = 10 * LAMPORTS_PER_SOL): Promise<Keypair> {
...
/**
 * Create multiple funded payers (for multi-player tests)
 */
export async function createFundedPlayers(count: number, lamportsEach: number = 10 * LAMPORTS_PER_SOL): Promise<Keypair[]> {
